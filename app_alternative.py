"""
Alternative Main Streamlit App for Real-Time Emotion-Based Music Recommendation
Uses Streamlit's built-in charts instead of Plotly for better compatibility
"""
...
def create_mood_history_chart():
    """Create real-time mood history chart using Streamlit's built-in charts"""
    if not st.session_state.mood_history:
        return None
    ...
    for entry in st.session_state.mood_history:
        ...
        emotion_data[emotion].append(entry['confidence'])
    ...
    chart_data = {}
    for emotion, confidences in emotion_data.items():
        chart_data[emotion.title()] = confidences
...
def main():
    ...
    with st.sidebar:
        ...
        if create_mood_history_chart():
            st.info("📈 Real-time emotion confidence tracking")
        else:
